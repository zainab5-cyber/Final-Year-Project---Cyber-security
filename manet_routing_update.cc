...
void RoutingExperiment::Run()
{
    ...
    // --- CSV for throughput logging --- // NEW
    std::ofstream throughputCSV("throughput.csv");
    throughputCSV << "Time,Bytes,Packets\n";
    throughputCSV.close();
    ...
    // --- Schedule CSV logging every second --- // NEW
    Simulator::Schedule(Seconds(1.0), [this]() {
        std::ofstream csv("throughput.csv", std::ios::app);
        csv << Simulator::Now().GetSeconds() << "," << this->bytesTotal
        ...
        // Reschedule
        Simulator::Schedule(Seconds(1.0), [this]() { CheckThroughput(); });
    });

    Simulator::Stop(Seconds(200));
    Simulator::Run();

    // --- Save FlowMonitor results --- // NEW
    flowmon->CheckForLostPackets();
    flowmon->SerializeToXmlFile("manet_ddos.flowmon", true, true);
